...
import com.uttlibrary.view.panel.ImportBookPanel;
import java.awt.*;
 ...
 * @author ADMIN
 */
public class ImportBookController extends JPanel {
    ...
    private void addImport() {
        try {
            ...
            if (msg.equals("SUCCESS")) {
                MessageBox.info(ImportBookPanel.this, "Nhập sách thành công!");
                loadTableData();
                clearForm();
            } else {
                MessageBox.error(ImportBookPanel.this, msg);
            }
        } catch (Exception e) {
            MessageBox.error(ImportBookPanel.this, "Vui lòng nhập số hợp lệ cho ID sách, số lượng và giá!");
        }
    }

    private void updateImport() {
        if (selectedId <= 0) {
            MessageBox.warning(ImportBookPanel.this, "Chọn một phiếu nhập để sửa!");
            return;
        }
        try {
            ImportBook ib = getFormData();
            ...
            if (msg.equals("SUCCESS")) {
                MessageBox.info(ImportBookPanel.this, "Cập nhật thành công!");
                loadTableData();
            } else {
                MessageBox.error(ImportBookPanel.this, msg);
            }
        } catch (Exception e) {
            MessageBox.error(ImportBookPanel.this, "Dữ liệu nhập vào không hợp lệ!");
        }
    }

    private void deleteImport() {
        if (selectedId <= 0) {
            MessageBox.warning(ImportBookPanel.this, "Chọn phiếu nhập cần xóa!");
            return;
        }
        if (!MessageBox.confirm(ImportBookPanel.this, "Bạn có muốn xóa phiếu nhập này không?")) return;
        
        String msg = controller.deleteImport(selectedId);
        if (msg.equals("SUCCESS")) {
            MessageBox.info(ImportBookPanel.this, "Đã xóa!");
            loadTableData();
            clearForm();
        } else {
            MessageBox.error(ImportBookPanel.this, msg);
